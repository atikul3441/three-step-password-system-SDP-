...
# Database Connection ---
def connect_db():
    conn = sqlite3.connect(DB, timeout=30)  # Wait up to 30 seconds if locked
...
def init_db():
    with connect_db() as conn:
        # fixed CREATE TABLE syntax and added new column card_values to store updated values separately
        conn.execute("""
...
def add_missing_columns():
    with connect_db() as conn:
        ...
        # include card_values as the new column required
        required_cols = {
            "first_name": "TEXT",
            "last_name": "TEXT",
            "passkey": "TEXT",
...
def step1_poker_security(user):
    ...
    # check if user already has passkey (treat None or empty string as not set)
    if not user["passkey"]:
        ...
        # compute passkey using rule: for each x compute 3*x+1, if result has two digits take last digit
        numbers = [card_values[c] for c in selection]
        results = []
        for x in numbers:
            val = 3 * x + 1
            if val >= 10:
                val = int(str(val)[-1])  # take last digit
            results.append(val)
        ...
        passkey = passkey[:pos] + random_digit + passkey[pos:]  # final 8-digit passkey
        ...
        # handle duplicate passkey by altering slightly (keeps length 8)
        with connect_db() as conn:
            cur = conn.cursor()
        ...
        # save to db: passkey, card_sequence (selection order), and hidden card_values mapping in card_values column
        with connect_db() as conn:
        ...
        # existing user — verification mode
        print("\n--- Step 1: Verify Your Poker Card Sequence ---")

        # get stored fields
        full_sequence_field = user["card_sequence"] or ""
        stored_passkey = user["passkey"] or ""
        stored_card_values_field = user["card_values"] or ""

        # parse stored selection sequence
        stored_sequence = [s.strip() for s in full_sequence_field.split(",") if s.strip()]

        # use only the first 7 digits (ignore the extra inserted random one)
        numeric_values = [int(d) for d in stored_passkey if d.isdigit()][:7]

        # map the numbers back to the user's cards secretly (this mapping is used internally)
        # If stored_sequence length < 7, pad or handle gracefully
        mapped_cards = stored_sequence[:7]
        card_values_by_passkey = dict(zip(mapped_cards, numeric_values))

        # Save updated card_values_by_passkey into the new card_values column (so you can see updated values in DB).
        # Format like "Spade:4, Heart:2, ..." — this is separate from the original hidden mapping.
        updated_value_text = ", ".join([f"{k}:{v}" for k, v in card_values_by_passkey.items()])

        # update database using connect_db (ensure updating same DB)
        with connect_db() as conn:
            cur = conn.cursor()
            # update the dedicated card_values column
            cur.execute("UPDATE users SET card_values=? WHERE username=?", (updated_value_text, user["username"]))
            conn.commit()

        # keep original stored_sequence for verification
        ...
        while True:
            # show full 3x3 matrix of cards shuffled (user must choose from all 9 cards)
            shuffled_cards = cards[:]  # full 9 cards
