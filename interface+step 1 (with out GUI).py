...
def step1_poker_security(user):
    ...
    if not user["passkey"]:
        ...
        # assign random values 0–8 to cards
        ...
        results = [3*x + 1 for x in numbers]
        ...
        with connect_db() as conn:
            ...
            if cur.fetchone():
                passkey = str(int(passkey) + random.randint(1, 9))[:7]
        ...
        with connect_db() as conn:
            conn.execute("UPDATE users SET passkey=?, card_sequence=? WHERE username=?",
                         (passkey, ",".join(selection), user["username"]))
            conn.commit()
        return True
        ...
        stored_sequence = user["card_sequence"].split(",")
        ...
        while True:
            random.shuffle(stored_sequence)
            print("\nSelect your 7 cards in correct sequence:")
            print(" | ".join(stored_sequence))
            print()
            attempt = []
            for i in range(7):
                choice = input(f"Select card #{i+1}: ").capitalize()
                attempt.append(choice)

            pass_input = input("Enter your 7-digit passkey: ").strip()

            if attempt == user["card_sequence"].split(",") and pass_input == stored_passkey:
